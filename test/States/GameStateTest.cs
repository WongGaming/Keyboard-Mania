        ...
        private bool CheckForHit(Note note, float hitPointY, int lane)
        {
            ...
            if (note.HitObject.IsHeldNote && _keysPressed[lane])
            {
                if (keyboardState.IsKeyUp(_keyMapping[lane]))
                {
                    ...
                    double holdDuration = _currentTime - note.HitObject.StartTime;
                    if (Math.Abs(holdDuration - note.HitObject.HoldDuration) <= _hitMargin)
                    {
                        note.CompleteHold();
                        _comboCount += (int)(holdDuration / 10); // Increment score for every 10 ms held
                        return true;
                    }
                    else
                    {
